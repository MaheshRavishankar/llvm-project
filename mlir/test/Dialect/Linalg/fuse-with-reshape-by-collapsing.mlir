...
func @fold_non_consecutive_reduction_dims(%arg0 : tensor<?x?xi32>) -> tensor<i32> {
  %c0 = arith.constant 0 : index
  %c2 = arith.constant 2 : index
  %0 = tensor.expand_shape %arg0 [[0, 1], [2, 3]] : tensor<?x?xi32> into tensor<?x4x?x8xi32>
  %d0 = tensor.dim %0, %c0 : tensor<?x4x?x8xi32>
...
//      CHECK: func @fold_non_consecutive_reduction_dims(
// CHECK-SAME:   %[[ARG0:.+]]: tensor<?x?xi32>)
//      CHECK:   %[[EXPAND_ARG0:.+]] = tensor.expand_shape %[[ARG0]] {{\[}}[0, 1], [2, 3]{{\]}}
//      CHECK:   %[[COLLAPSE_ARG0:.+]] = tensor.collapse_shape %[[EXPAND_ARG0]] {{\[}}[0, 1], [2], [3]{{\]}}
//      CHECK:   %[[GENERIC:.+]] = linalg.generic
// CHECK-SAME:       indexing_maps = [#[[MAP0]], #[[MAP1]]]
// CHECK-NEXT:   ^bb{{[0-9]}}
//      CHECK:       %[[ID0:.+]] = linalg.index 0
...
//  CHECK-DAG:       %[[T1:.+]] = arith.divui %[[ID0]], %[[C4]]
//      CHECK:       %[[ID1:.+]] = linalg.index 1
//  CHECK-DAG:       %[[ID2:.+]] = linalg.index 2
//  CHECK-DAG:       %[[T2:.+]] = arith.addi %[[T1]], %[[ID1]]
//  CHECK-DAG:       %[[T3:.+]] = arith.addi %[[T2]], %[[T0]]
//  CHECK-DAG:       %[[T4:.+]] = arith.addi %[[T3]], %[[ID2]]
//  CHECK-DAG:       %[[T5:.+]] = arith.index_cast %[[T4]]

...
struct SCFTilingOptions {
  ...
  /// Specify the reduction dimensions to be used.
  SetVector<unsigned> reductionDims;
  SCFTilingOptions &
  setReductionDims(ArrayRef<unsigned> dims) {
    reductionDims.insert(dims.begin(), dims.end());
    return *this;
  }

  /// Set payload used for partial reduction tiling. This payload
  /// is not used by the tiling algorithm itself, but is forward to
  /// the `PartialReductionTilingInterface` methods for use by those
  /// methods.
  void *partialReductionPayload = nullptr;
  SCFTilingOptions &
  setPartialReductionTilingPayload(void *payload) {
    partialReductionPayload = payload;
    return *this;
  }
};

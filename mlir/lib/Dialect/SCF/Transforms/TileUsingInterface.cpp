...
mlir::scf::tileReductionUsingScf(RewriterBase &b,
  ...
  // 1. Find destination tensors to use for the operation.
  SmallVector<Value> destinationTensors;
  if (failed(tensor::getOrCreateDestinations(b, op.getLoc(), op,
                                             destinationTensors))) {
    return b.notifyMatchFailure(op, "unable to create destination tensors");
  }

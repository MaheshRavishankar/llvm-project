...
/// Non-existense of the sequence returns true as well.
...
static bool isFusableWithReshapeByDimCollapse(
  ...
  for (ReassociationIndicesRef foldedIterDims : iterationReassociation) {
    // Check that all indexing maps, the folded dimensions sequence is
    // preserved.
...
private:
  ...
  /// Map from the starting dimension folded to the sequence of folded
  /// dimension. They value refers to the position in `iterationReassociation`.
...
/// Compute the indexing map the collapsed op that corresponds to the given
/// `indexingMap` of the original operation.

...
struct LinalgPartialReductionTilingPayload {
  SmallVector<AffineMap> partialReductionMaps;
};

static LinalgPartialReductionTilingPayload
getOrDefaultLinalgPartialReductionTilingPayload(
    LinalgOp linalgOp, const SetVector<unsigned> &reductionDims,
    void *payload) {
  if (payload) {
    return *(static_cast<LinalgPartialReductionTilingPayload *>(payload));
  }
  ...
  return LinalgPartialReductionTilingPayload{std::move(partialReductionMaps)};
}
...
static Operation *getInitSliceForOuterParallel(
    OpBuilder &b, Location loc, Value initValue, ArrayRef<OpFoldResult> offsets,
    ArrayRef<OpFoldResult> sizes, const SetVector<unsigned> &reductionDims,
    AffineMap partialReductionMap) {
  int64_t initRank = partialReductionMap.getNumResults();
  ...
  dispatchIndexOpFoldResults(resultShape, dynamicShapes, staticShapes);
  Type elementType = getElementTypeOrSelf(initValue.getType());
  auto sliceType = RankedTensorType::get(staticShapes, elementType);
  auto extractSlice = b.create<tensor::ExtractSliceOp>(
      loc, sliceType, initValue, initOffsets, initSizes, initStrides);
  return extractSlice;
}
...
struct LinalgOpPartialReductionInterface
  ...
  FailureOr<SmallVector<Value>> generateInitialTensorForPartialReduction(
      ...
      const SetVector<unsigned> &reductionDims, void *payload) const {
    auto linalgOp = cast<LinalgOp>(op);
    ...
    LinalgPartialReductionTilingPayload linalgPayload =
        getOrDefaultLinalgPartialReductionTilingPayload(linalgOp, reductionDims,
                                                        payload);
    ArrayRef<AffineMap> partialResultMaps(linalgPayload.partialReductionMaps);
  ...
  FailureOr<TilingResult> tileToPartialReduction(
    ...
    LinalgPartialReductionTilingPayload linalgPayload =
        getOrDefaultLinalgPartialReductionTilingPayload(linalgOp, reductionDims,
                                                        payload);
    ArrayRef<AffineMap> partialReductionMaps(
        linalgPayload.partialReductionMaps);
    ...
    for (auto [partialReductionMap, valueToTile] :
      ...
      Operation *sliceOp =
          (tilingStrategy ==
           ReductionTilingStrategy::PartialReductionOuterReduction)
              ? getInitSliceForOuterReduction(b, loc, valueToTile, offsets,
                                              sizes, reductionDims,
                                              partialReductionMap)
              : getInitSliceForOuterParallel(b, loc, valueToTile, offsets,
                                             sizes, reductionDims,
                                             partialReductionMap);
  ...
  LogicalResult getPartialResultTilePosition(
      Operation *op, OpBuilder &b, unsigned resultNumber,
      ArrayRef<OpFoldResult> offsets, ArrayRef<OpFoldResult> sizes,
      const SetVector<unsigned> &reductionDims, void *payload,
      SmallVector<OpFoldResult> &resultOffsets,
      SmallVector<OpFoldResult> &resultSizes) const {
    auto linalgOp = cast<LinalgOp>(op);
    LinalgPartialReductionTilingPayload linalgPayload =

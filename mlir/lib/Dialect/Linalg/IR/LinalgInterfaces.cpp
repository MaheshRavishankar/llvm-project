...
struct ConvAccessExprWalker
  ...
  LogicalResult visitAffineBinaryOpExpr(AffineBinaryOpExpr binaryExpr) {
    ...
    if (succeeded(isDimExprOrMulExpr(binaryExpr.getLHS())) &&
...
static MatchConvolutionResult isConvolutionInterfaceImpl(Operation *op) {
  ...
  // Make sure all loops are charecterized.
  llvm::SmallDenseSet<unsigned> allLoopDims;
  // Batch loop : present in output, as non-convolved in input, not present in
  // filter. Output image dimension : present in output, convolved dims in
  // input, not present in filter. Output channel dimension : present in output,
  // not present in input, present in filter. Filter loop dimension : present in
  // filter, convolved in input, not present in output. Input channel dimension
  // : unconvolved in input, not present in output, present in filter. depthwise
  // dimension : unconvolved in input, present in output, present in filter.
...
LogicalResult mlir::linalg::detail::verifyConvolutionInterface(Operation *op) {
  ...
  if (res == MatchConvolutionResult::NonConvolutionLoop) {
    return op->emitError("unexpected dimension for convolution op");
  }
...
Optional<unsigned> mlir::getConvolutionBatchLoopDim(Operation *op) {
  ...
  // The batch dimensions are part of unconvoled dimensions in the input, and
...
SmallVector<unsigned> mlir::getConvolutionOutputImageLoopDims(Operation *op) {
  auto linalgOp = cast<linalg::LinalgOp>(op);
  auto indexingMaps = linalgOp.getIndexingMaps();
  ConvAccessExprWalker inputExprWalker;
  if (llvm::any_of(indexingMaps[0].getResults(),
  ...
  SmallVector<unsigned> outputImageLoopDims;
  for (AffineExpr expr : indexingMaps.back().getResults()) {
    unsigned dim = expr.cast<AffineDimExpr>().getPosition();
    if (inputExprWalker.convolvedDims.count(dim))
      outputImageLoopDims.push_back(dim);
  }
  return outputImageLoopDims;
}

SmallVector<unsigned> mlir::getConvolutionFilterLoopDims(Operation *op) {
  auto linalgOp = cast<linalg::LinalgOp>(op);
  auto indexingMaps = linalgOp.getIndexingMaps();
  ConvAccessExprWalker inputExprWalker;
  if (llvm::any_of(indexingMaps[0].getResults(),
  ...
  // the output.
  SmallVector<unsigned> filterLoopDims;
  for (AffineExpr expr : indexingMaps[1].getResults()) {
    unsigned dim = expr.cast<AffineDimExpr>().getPosition();
    if (inputExprWalker.convolvedDims.count(dim))
      filterLoopDims.push_back(dim);
  }
  return filterLoopDims;
}

Optional<unsigned> mlir::getConvolutionOutputChannelLoopDim(Operation *op) {
  auto linalgOp = cast<linalg::LinalgOp>(op);
  auto indexingMaps = linalgOp.getIndexingMaps();
  ConvAccessExprWalker inputExprWalker;
  if (llvm::any_of(indexingMaps[0].getResults(),
  ...
  // The output channel dimensions are not part of convolved or unconvolved
  // dimensions in the input, and present in the filter.
  llvm::SmallDenseSet<unsigned> filterDims = getPreservedDims(indexingMaps[1]);
  for (AffineExpr expr : indexingMaps.back().getResults()) {
    unsigned dim = expr.cast<AffineDimExpr>().getPosition();
    if (!inputExprWalker.convolvedDims.count(dim) &&
        !inputExprWalker.unConvolvedDims.count(dim) && filterDims.count(dim))
      return dim;
  }
  return llvm::None;
}

Optional<unsigned> mlir::getConvolutionInputChannelLoopDim(Operation *op) {
  auto linalgOp = cast<linalg::LinalgOp>(op);
  auto indexingMaps = linalgOp.getIndexingMaps();
  ConvAccessExprWalker inputExprWalker;
  if (llvm::any_of(indexingMaps[0].getResults(),
...
Optional<unsigned> mlir::getConvolutionDepthwiseLoopDim(Operation *op) {
  auto linalgOp = cast<linalg::LinalgOp>(op);
  auto indexingMaps = linalgOp.getIndexingMaps();
  ConvAccessExprWalker inputExprWalker;
  if (llvm::any_of(indexingMaps[0].getResults(),
